...
8. **Tag strip**: one scrolling row of chips under the slider, no header, like the word suggestions above a keyboard. First a chevron chip (**Tutti i tag**), then **every enabled tag**, the most used first (count over every entry, ties in vocabulary order; a fresh install shows vocabulary order). What gets used sits under the thumb, the long tail is a swipe away, and nothing needs a cutoff or a "keep the selected ones visible" rule. Tap toggles the tag on the draft; the order never changes under a finger, it follows usage across saves (#4, #15). The chevron replaces the strip with the same tags grouped by type, for browsing by category; the same chip (now pointing up) on the first group's title row folds it back, and a new draft (save, Azzera, another entry to edit) folds it too.
