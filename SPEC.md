...
8. **Tag strip**: one row of tag chips under the slider, no header, like the word suggestions above a keyboard. First a chevron chip (**Tutti i tag**), then the 6 most used tags (count over every entry, ties in vocabulary order; a fresh install shows the first enabled tags in vocabulary order), so the user meets the tags without opening anything. Tap toggles the tag on the draft (#4). A tag set from the full list that is not among the six is appended to the strip, pressed, so it can be toggled off there; appended rather than moved to the front, so the strip never reorders under a finger (#15). The chevron replaces the strip with every enabled tag grouped by type, the same chip (now pointing up) at the top folding it back; a new draft (save, Azzera, another entry to edit) folds it too.
