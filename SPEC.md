...
- **Areas.** `type Area = { regions: RegionId[]; intensity: number; strokes?: Stroke[] }`. An entry has zero or more areas, each a set of regions sharing one level, optionally shaded by hand inside its body regions (§5.3). `readings.pain` is the max over the areas with a body in them when there are any, 0 when only the mind is selected (§5.3), otherwise a free value (used for "no pain today" or entries with no location).
- An area with `*` among its regions means full body and is always the only area; the mind (`mind`) may sit beside the star. The mind is one region among the others (§5.4); an area holding nothing but the mind has no pain, so its level is the highest mental reading instead (#28). Entries written before 0.5.0 always kept the mind in an area of its own; they stay as they are.
...
- Tapping a region that belongs to another area moves it into the current one, with its paint (§5.3). Tapping a region in the current area removes it, with its paint; so does a limb or a set chip. Whenever a tap takes paint away, a toast "Tratti cancellati · Annulla" offers to undo it.
- Full body replaces all areas with a single one, keeping their paint and the mind; off again, the mind stays selected if it was.
- **The mind is one region among the others** (#28): tapping it puts it in the current area or takes it out, like a body part, so body and mind make one pill, "5 · gambe, mente", at the body's level. Mirror never applies to it, sets and limbs never include it, and it stays toggleable under full body. An area holding only the mind has no pain: its level is not set by the slider but is the highest mental reading (§6.1), so the chip reads "6 · mente"; the moment a body part joins it, the area takes the pain level and the slider edits it. Before 0.5.0 the mind was always alone in its area, which made two pills of one selection; the tester's picture (#16) is one.
- Overall pain shown in the diary and used for trends is the max over the areas with a body in them; 0 when only the mind is selected, whatever the pain slider showed before the mind hid it.
...
- Each save logs an **ordinary moment** carrying `preset`: its areas at the pain level (one holding only the mind at the highest mental one), its tags, an empty note, an episode when `ongoing` is set (the existing card handles the end). The diary stays honest and every existing feature works.
...
4. **Body map**, front and back side by side with the mind between them (§5.3), "both sides" toggle, full body / legs / arms / **Disegna** chips (§5.3). While drawing, the map's slot holds one enlarged figure, then the front / back chips with **Annulla tratto** and **Cancella disegno**, and a one-line hint. Area chips under the map (§5.4).
...
7. **Intensity slider**: large, full width, 0..10 with the number shown big and a colour ramp. Snaps to integers. Drag or tap. Hidden while only the mind is selected (item 9). It edits the current area, or the last one with a body in it while an area holding only the mind is current.
...
- **Body heatmap**: the same body SVG, regions coloured by how often and how intensely they appeared in range, every stroke in range laid over it at low opacity in its level's colour (§5.3); the mind is one more region, coloured by the mental readings of the entries that selected it (their highest one each), since its area's level is the body's (§5.4).
